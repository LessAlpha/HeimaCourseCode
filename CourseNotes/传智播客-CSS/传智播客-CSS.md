...
（4）行内元素只能容纳文本或其他行内元素。（a特殊:a里面可以放块级元素 ）

（5）代码换行被解析（间隔大小取决于父级的font-size属性值，浮动元素的间隔大小为0）
...
（4）代码换行被解析（非浮动元素的间隔大小取决于父级的font-size属性值，浮动元素的间隔大小为0）
...
交集选择器由两个选择器构成，其中第一个为标签选择器，第二个为class选择器，两个选择器之间**不能有空格**，如h3.special。
...
ele1 + ele2     ele1元素之后紧挨着的ele2兄弟元素

ele1 ~ ele2     ele1元素之后不必紧挨着的ele2兄弟元素
...
1、宽度属性width和高度属性height仅适用于块级和行内块元素，对行内元素无效（ img 标签和 input除外）。
...
1.  定位属性为absolute的元素一定要给top,left或bottom,right属性显性赋值
2.  top,left的优先级高于bottom,right
...
**因此 比如 行内元素 如果添加了 绝对定位或者 固定定位或者 浮动后（不包括相对定位），可以不用转换模式，直接给高度和宽度就可以了。**
...
<img src="media/vertical-align控制行内(块)元素的对齐方式1.png" />

<img src="media/vertical-align控制行内(块)元素的对齐方式2.png" />
